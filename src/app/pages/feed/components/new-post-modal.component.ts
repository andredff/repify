      ...
      <div class="flex-1 overflow-y-auto p-5 space-y-5">
          ...
          @if (photoPreview()) {
            <div class="relative rounded-2xl overflow-hidden bg-card-2">
              ...
              @if (autoGeneratedVisual()) {
                <div class="absolute left-3 top-3 rounded-full border border-primary/25 bg-bg/82 px-3 py-1.5 text-[10px] font-body font-semibold uppercase tracking-[0.18em] text-primary backdrop-blur-sm">
                  Repify Challenge
                </div>
              }
...
export class NewPostModalComponent {
  ...
  private async buildChallengeArtwork(summary: WorkoutPostPrefillSummary): Promise<{ dataUrl: string; file: File }> {
    ...
    const exerciseProgress = summary.totalExercises > 0
      ...
      : 0;
    const dateLabel = summary.sessionLabel === 'Hoje' ? 'Treino de hoje' : summary.sessionLabel;
    const metrics = [
    ...
    fillRoundedRect(64, 56, 260, 52, 26, 'rgba(255,255,255,0.06)');
    strokeRoundedRect(64, 56, 260, 52, 26, 'rgba(255,255,255,0.1)', 2);
    context.font = '700 24px "Arial", sans-serif';
    context.fillStyle = '#EAF2FF';
    context.fillText('REPIFY CHALLENGE', 92, 90);

    fillRoundedRect(748, 56, 268, 52, 26, 'rgba(0,255,136,0.12)');
    strokeRoundedRect(748, 56, 268, 52, 26, 'rgba(0,255,136,0.22)', 2);
    context.font = '700 24px "Arial", sans-serif';
    context.fillStyle = '#00FF88';
    context.fillText(dateLabel.toUpperCase(), 778, 90);
    ...
    context.fillStyle = 'rgba(0,194,255,0.95)';
    context.fillText(summary.workoutType.toUpperCase(), 102, 496);

    context.font = '900 122px "Arial Black", sans-serif';
    context.fillStyle = '#FFFFFF';
    context.fillText(`+${summary.xpEarned}`, 96, 622);
    context.font = '700 36px "Arial", sans-serif';
    context.fillStyle = '#00FF88';
    context.fillText('XP GANHO', 102, 664);

    context.font = '700 22px "Arial", sans-serif';
    context.fillStyle = 'rgba(234,242,255,0.78)';
    const athleteName = profile.full_name.trim() || profile.username.trim() || 'Seu treino';
    context.fillText(athleteName.toUpperCase(), 102, 560);
    ...
    context.fillStyle = 'rgba(234,242,255,0.75)';
    context.fillText('EXECUCAO DO TREINO', 686, 526);
    ...
    context.font = '700 18px "Arial", sans-serif';
    context.fillStyle = 'rgba(234,242,255,0.82)';
    context.fillText(`${summary.exercisesDone}/${summary.totalExercises} exercicios fechados`, 686, 662);

    context.font = '800 52px "Arial Black", sans-serif';
    context.fillStyle = '#FFFFFF';
    const wrappedTitle = wrapText(summary.title.toUpperCase(), 932);
    wrappedTitle.slice(0, 2).forEach((line, index) => {
      context.fillText(line, 74, 790 + (index * 62));
    });

    context.font = '700 26px "Arial", sans-serif';
    context.fillStyle = 'rgba(234,242,255,0.68)';
    context.fillText(`${summary.muscleGroup.toUpperCase()}  •  ${summary.difficulty.toUpperCase()}  •  ${summary.completedAtLabel}`, 76, 918);
    ...
    metrics.forEach((metric, index) => {
      ...
      context.font = '700 18px "Arial", sans-serif';
      context.fillStyle = 'rgba(0,255,136,0.82)';
      context.fillText(metric.label.toUpperCase(), x + 24, y + 34);
      context.font = '800 32px "Arial Black", sans-serif';
      context.fillStyle = '#F5FAFF';
      context.fillText(metric.value.toUpperCase(), x + 24, y + 74);
    });
