...
async function main() {
    ...
    const [historyBefore, daySessionsBefore, programsBefore] = await Promise.all([
        supabase_1.supabaseAdmin.from('workout_history').select('*', { count: 'exact', head: true }),
        supabase_1.supabaseAdmin.from('workout_day_sessions').select('*', { count: 'exact', head: true }),
        supabase_1.supabaseAdmin.from('workout_programs').select('*', { count: 'exact', head: true }),
    ]);
    const [{ count: workoutEventsBefore, error: beforeError }, usersResult] = await Promise.all([
        supabase_1.supabaseAdmin.from('xp_events').select('*', { count: 'exact', head: true }).eq('type', 'workout'),
        listAllUsers(),
    ]);
    if (beforeError)
        throw beforeError;
    const users = usersResult;
    const { error: deleteError } = await supabase_1.supabaseAdmin.from('xp_events').delete().eq('type', 'workout');
    if (deleteError)
        throw deleteError;
    const { error: deleteHistoryError } = await supabase_1.supabaseAdmin.from('workout_history').delete().neq('id', '00000000-0000-0000-0000-000000000000');
    if (deleteHistoryError)
        throw deleteHistoryError;
    const { error: deleteDaySessionsError } = await supabase_1.supabaseAdmin.from('workout_day_sessions').delete().neq('user_id', '00000000-0000-0000-0000-000000000000');
    if (deleteDaySessionsError)
        throw deleteDaySessionsError;
    const { error: deleteProgramsError } = await supabase_1.supabaseAdmin.from('workout_programs').delete().neq('user_id', '00000000-0000-0000-0000-000000000000');
    if (deleteProgramsError)
        throw deleteProgramsError;
    let processed = 0;
    for (const user of users) {
        const { data: remainingEvents, error: remainingError } = await supabase_1.supabaseAdmin
            .from('xp_events')
        ...
        const { error: upsertError } = await supabase_1.supabaseAdmin.from('user_stats').upsert({
            user_id: user.id,
            total_xp: totalXp,
            weekly_xp: weeklyXp,
        ...
        const { error: updateUserError } = await supabase_1.supabaseAdmin.auth.admin.updateUserById(user.id, {
    ...
    if (afterError)
        throw afterError;
    if (verificationStats.error)
        throw verificationStats.error;
